...
var orm = module.exports = {
  ...
  getActiveErrors: function(vendor) {
    var query = db.queries.getActiveErrors;
    if(typeof vendor !== 'undefined') {
      query += ' AND e.store_id = 901';
    } else {
      query += ' AND e.store_id != 901';
    }
    return new Promise(function(resolve, reject){
  ...
  getStores: function(action, values, vendor) {
    var condition = '';
    if(typeof vendor !== 'undefined') {
      if (action === 'exclude') {
        condition = 'WHERE s.id NOT IN ('+values.join(", ")+')';
      } else {
        condition = 'WHERE s.id IN (901, 902, 903)';
      }
    } else {
      if(action === 'include') {
  ...
  cleanPings: function(store) {
    return new Promise(function(resolve, reject) {
      var query = db.queries.getValidPings;
      query = query.replace('"%store_id%"', store);
      orm.connection.query(query, function(error, results, fields) {
        if(results.length > 0) {
          query = db.queries.cleanupPings;
          query = query.replace('"%store_id%"', store);
          query = query.replace('"%ping_id%"', results[0].id);
          orm.connection.query(query, function(error, results, fields) {
            if(error) {
              reject(error);
            }
  ...
  getTwoLastCassettes: function(store) {
    return new Promise(function(resolve, reject) {
      var query = db.queries.getTwoLastCassettes;
      query = query.replace('"%store_id%"', store);
      orm.connection.query(query, function(error, results, fields) {
        if(error) {
          reject(error);
